...
void KrigingAlgo::Kriging2::SimpleKrige(Model model, cli::array<float>^ buffer, double nugget, double sill, double range)
{
	...
	switch (model)
	{
	case KrigingAlgo::Kriging2::Model::Linear:
		v.VAR = CIRCULAR;
		break;
	case KrigingAlgo::Kriging2::Model::LinearWithoutIntercept:
		v.VAR = RATIONAL_Q;
		break;
	case KrigingAlgo::Kriging2::Model::Spherical:
		v.VAR = SPHERICAL;
		break;
	case KrigingAlgo::Kriging2::Model::Exponential:
		v.VAR = EXPONENTIAL;
		break;
	case KrigingAlgo::Kriging2::Model::Gaussian:
		v.VAR = GAUSSIAN;
		break;
	default:
		v.VAR = GAUSSIAN;
